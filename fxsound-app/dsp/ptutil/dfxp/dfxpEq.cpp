...
#ifdef NO_REGISTRY
	// No registry available - initialize from memory (default to IS_TRUE)
	cast_handle->eq.i_processing_on = IS_TRUE;
#else
...
#ifdef NO_REGISTRY
	// No registry available - return the in-memory value
	*ip_on = cast_handle->eq.i_processing_on;
	return(OKAY);
#else
	swprintf(wcp_full_key_path, L"%s\\%s\\%d\\%d\\%s\\%s\\%s", 
...
#ifdef NO_REGISTRY
	// No registry available - return the in-memory processing value instead
	if (GraphicEqGetBandBoostCut(cast_handle->eq.graphicEq_hdl, i_band_num, rp_boost_cut) != OKAY)
		return(NOT_OKAY);
	swprintf(wcp_boost_cut, L"%.2f", *rp_boost_cut);
	return(OKAY);
