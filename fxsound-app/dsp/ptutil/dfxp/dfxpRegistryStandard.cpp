...
{
#ifdef NO_REGISTRY
	// No registry available - no installation date
	*ip_date_exists = IS_FALSE;
	return(OKAY);
#else
...
#ifdef NO_REGISTRY
	// No registry available - no last used date
	return(OKAY);
#else
	wchar_t wcp_full_key_path[PT_MAX_PATH_STRLEN];
	int is_long;
...
#ifdef NO_REGISTRY
	// No registry available - last used date is not recorded
	return(OKAY);
#else
...
#ifdef NO_REGISTRY
	// No registry available - return empty path
	return(OKAY);
#else
	wchar_t wcp_full_key_path[PT_MAX_PATH_STRLEN];
	int key_exists;
