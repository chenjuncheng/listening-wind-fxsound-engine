...
#ifdef NO_REGISTRY
	// No registry available - nothing to update from, all state is in memory
	return(OKAY);
#else
...
int DfxDspPrivate::resetEQ()
{
	...
	{
#ifdef NO_REGISTRY
		if (dfxpEqSetBandBoostCut(dfxp_handle_, DFXP_STORAGE_TYPE_MEMORY, i_band_num, (realtype)0.0) != OKAY)
			return(NOT_OKAY);
#else
	...
	*/
...
#ifdef NO_REGISTRY
		if (dfxpEqSetProcessingOn(dfxp_handle_, DFXP_STORAGE_TYPE_MEMORY, i_eq_on) != OKAY)
			return(NOT_OKAY);
#else
...
#ifdef NO_REGISTRY
			if (dfxpEqSetBandBoostCut(dfxp_handle_, DFXP_STORAGE_TYPE_MEMORY, i_band_num, r_boost_cut) != OKAY)
				return(NOT_OKAY);
#else
	...
	else
	{
		...
		if (nBands != graphic_eq_num_bands)
		{
			...
			{
#ifdef NO_REGISTRY
				if (dfxpEqSetBandBoostCut(dfxp_handle_, DFXP_STORAGE_TYPE_MEMORY, i_band_num, r_boost_cut_interpolated[i_band_num]) != OKAY)
					return(NOT_OKAY);
#else
		...
		else
		{
			...
			{
#ifdef NO_REGISTRY
				if (dfxpEqSetBandBoostCut(dfxp_handle_, DFXP_STORAGE_TYPE_MEMORY, i_band_num, r_boost_cut_original[i_band_num]) != OKAY)
					return(NOT_OKAY);
#else
...
#ifdef NO_REGISTRY
	// No registry available - return the in-memory value regardless of requested storage type
	*ip_on = eq_processing_on_;
	return(OKAY);
#else
	swprintf(wcp_full_key_path, L"%s\\%s\\%d\\%d\\%s\\%s\\%s",
...
#ifdef NO_REGISTRY
	dfxpEqSetBandBoostCut(dfxp_handle_, DFXP_STORAGE_TYPE_MEMORY, band_num + 1, boost);
#else
