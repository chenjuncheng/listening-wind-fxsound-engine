...
#ifdef NO_REGISTRY
	// Skip first-run check in no-registry mode; just use current default device
...
PlaybackDeviceIsSelected:  // Label to jump to when the playback device num has been determined.
	...
	// Capture from the real playback device instead.
	cast_handle->captureDeviceNum = cast_handle->playbackDeviceNum;
#else
...
#ifdef FXSOUND_ENGINE_MVP
	// WASAPI Loopback mode: Use the playback device's sample rate directly.
	// No need to set DFX device sample rate since we're capturing from the real device.
...
#ifndef FXSOUND_ENGINE_MVP
	// Original FxSound: Set the system default device to the virtual DFX device so audio gets routed through it.
