...
{  
  ...
  {
	  // Don't restart the processing thread when DFX itself is set as default.
	  // This prevents the re-init storm loop: set DFX default → callback → stop → reinit → repeat.
	  if (pwstrDeviceId != NULL && cast_handle->dfxDeviceNum != SND_DEVICES_DEVICE_NOT_PRESENT)
		  if (wcscmp(pwstrDeviceId, cast_handle->pwszID[cast_handle->dfxDeviceNum]) == 0)
			  return(S_OK);

	  // Only restart if the default was changed AWAY from the DFX device (user manually switched).
	  // If DFX was the previous default and user switched away, we need to stop processing.
	  // If DFX was NOT the default, a default change doesn't affect our capture pipeline.
	  if (cast_handle->defaultDeviceNum != SND_DEVICES_DEVICE_NOT_PRESENT &&
		  cast_handle->defaultDeviceNum == cast_handle->dfxDeviceNum)
	  {
		  SLOUT_FIRST_LINE(L"CsndDevicesMMNotificationClient::OnDefaultDeviceChanged() default changed away from DFX, stopping");
		  cast_handle->stopAudioCaptureAndPlaybackLoop = 1;
	  }
	  // Otherwise: default changed but DFX wasn't the default, or we can't determine.
	  // Don't trigger re-init — our pipeline is still valid.
  }
...
#ifdef FXSOUND_ENGINE_MVP
  // In Loopback mode, suppress device change notifications to avoid callback storms
  return S_OK;
#endif

  // Only notify device changes when we're not ignoring callbacks (i.e. not during init/reinit)
...
HRESULT STDMETHODCALLTYPE CsndDevicesMMNotificationClient::OnDeviceAdded(LPCWSTR pwstrDeviceId)
{
  ...
  if( cast_handle->ignoreDeviceCallbacks == FALSE )
  {
	  // Don't restart the processing thread when a device is added.
	  // New devices don't invalidate our current capture/playback sessions.
	  // The next sndCheckDeviceChanges() poll will pick it up if needed.
  }
  ...
  // Only notify device changes when we're not ignoring callbacks
  if (cast_handle->ignoreDeviceCallbacks == FALSE)
  {
	  if (cast_handle->deviceChangeCallback != NULL)
		  cast_handle->deviceChangeCallback();
                          ...
                          DWORD dwNewState)
{
  ...
  if( cast_handle->ignoreDeviceCallbacks == FALSE )
  {
	  // Only restart the processing thread when a critical device is lost.
	  // DEVICE_STATE_ACTIVE / DEVICE_STATE_DISABLED can be triggered by our own WASAPI session
	  // setup, causing a re-init storm loop. Only stop on device removal.
	  if (dwNewState == DEVICE_STATE_NOTPRESENT || dwNewState == DEVICE_STATE_UNPLUGGED)
	  {
		  SLOUT_FIRST_LINE(L"CsndDevicesMMNotificationClient::OnDeviceStateChanged() device lost, stopping");
		  cast_handle->stopAudioCaptureAndPlaybackLoop = 1;
	  }
