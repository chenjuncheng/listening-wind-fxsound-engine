...
int PT_DECLSPEC sndDevicesReInit(PT_HANDLE *hp_sndDevices, int i_initType, int *ipNumRealDevices, int *ipDfxDeviceEnabledFlag, int *ip_status)
{
	...
	*ipDfxDeviceEnabledFlag = IS_FALSE;
	...
	{	// The SND_DEVICES_INIT_FOR_PROCESSING and SND_DEVICES_INIT_NO_PROCESSING cases.
		...
		// WASAPI Loopback mode: No virtual DFX device needed
		// Always set DfxDeviceEnabledFlag to IS_TRUE so the processing thread will start
		*ipDfxDeviceEnabledFlag = IS_TRUE;

		// Get all current devices (no need to loop/retry for DFX device)
...
#ifdef FXSOUND_ENGINE_MVP
			// WASAPI Loopback mode: capture and playback are the same device.
			// No need to sync volume between capture and playback.
#else
...
#ifdef NO_REGISTRY
			// In no-registry mode, preserve the user-set buffer size from setBufferLength()
			// (which writes directly to cast_handle->bufferSizeMilliSecs).
			// Only apply the default if it hasn't been set yet.
			if (cast_handle->bufferSizeMilliSecs <= 0)
...
#ifdef NO_REGISTRY
			// Use default buffer size in no-registry mode
#else
...
int PT_DECLSPEC sndCheckDeviceChanges(PT_HANDLE* hp_sndDevices, BOOL* bp_deviceChanged)
{
    ...
    *bp_deviceChanged = FALSE;
	...
	// Don't trigger re-init if we're currently ignoring callbacks (e.g. during initialization)
	if (cast_handle->ignoreDeviceCallbacks == TRUE)
		return(OKAY);
	...
	// If count is different, just report the change — don't force stop.
	// The engine will decide whether to reinitialize based on the changed flag.
