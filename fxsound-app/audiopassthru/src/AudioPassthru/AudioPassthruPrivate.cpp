...
int AudioPassthruPrivate::sndDeviceHandleToSoundDevices(bool active_devices)
{
	...
	for (int index = 0; index < cast_handle->totalNumDevices; index++) 
	{
		...
		if (sound_device.pwszID == wcp_targeted_real_playback_device_guid)
		{
			...
			sound_device.isPlaybackDevice = true;
		}
		...
		// In Loopback mode, real devices that are not capture devices are playback devices
#ifdef FXSOUND_ENGINE_MVP
		if (sound_device.isRealDevice && !sound_device.isCaptureDevice)
		{
			sound_device.isPlaybackDevice = true;
		}
#endif
	...
	*/
	...
	if (b_need_to_start_thread)
	{
		...
		*/
		...
		// Notify callback of device list after re-init (only when thread was actually restarted)
		if (s_callback_ != nullptr && b_need_to_start_thread && hProcessingThread_ != NULL)
...
{
#ifdef FXSOUND_ENGINE_MVP
	// WASAPI Loopback mode: We never changed the default device, nothing to restore.
#else
	int i_resultFlag;
...
#ifdef FXSOUND_ENGINE_MVP
	// WASAPI Loopback mode: Store the targeted device and set it as the playback device directly.
	// Don't change the system default device - just update the internal playback device selection.
	// Store GUID in member variable so it persists beyond this function scope.
	swprintf(wcp_playback_device_guid_, PT_MAX_GENERIC_STRLEN, L"%s", sound_device_guid.c_str());

	// Set the targeted playback device directly (updates playbackDeviceNum internally)
	if (sndDevicesSetDeviceType(hp_sndDevices_, SND_DEVICES_TARGETED_REAL_PLAYBACK, wcp_new_targeted_real_playback_guid, &i_resultFlag) != OKAY)
		return(NOT_OKAY);

	// Kill the processing thread so it will reinit with the new device on next processTimer()
	int i_timed_out;
	if (killProcessingThread(&i_timed_out) != OKAY)
		return(NOT_OKAY);
#else
